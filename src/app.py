...
with chunked_col:
    ...
    for chunk_key, chunk in chunked_code_dict.items():
        # Support both new dict format and legacy strings
        if isinstance(chunk, dict):
            st.caption(f"Chunk {chunk_key} — lines {chunk.get('start_line')}-{chunk.get('end_line')}")
            st.code(chunk.get('text', ''), language=language)
        else:
            st.caption(f"Chunk {chunk_key}")
            st.code(str(chunk), language=language)
